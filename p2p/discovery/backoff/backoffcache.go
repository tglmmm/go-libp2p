...
// 它是一个对发现结果进行缓存和降低重复查询频率的实现
type BackoffDiscovery struct {
	disc         discovery.Discovery
	stratFactory BackoffFactory           // 退避策略,退避策略有助于减少对频繁查询的节点的负载，避免过度消耗资源
	peerCache    map[string]*backoffCache // 用于存储对等节点的发现结果
	peerCacheMux sync.RWMutex

	parallelBufSz int // 并行查询的缓冲区大小。它允许在并行查询中处理多个节点，以提高查询效率和并发性能。
	returnedBufSz int // 表示查询结果返回的缓冲区大小。它限制了一次性返回的查询结果的数量，以控制内存使用和处理负载。
