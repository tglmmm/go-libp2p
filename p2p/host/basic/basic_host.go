...
//
// 使用协议多路复用器（protocol muxer）来多路复用每个协议的流（stream）。
// 使用身份验证服务（identity service）来发送和接收节点信息。
// 使用 NAT 服务（NAT service）来建立 NAT 端口映射。
type BasicHost struct {
	ctx       context.Context
	ctxCancel context.CancelFunc
	// ensures we shutdown ONLY once
	closeSync sync.Once // 确保只关闭一次
	// keep track of resources we need to wait on before shutting down
	refCount sync.WaitGroup // 开启的其他携程的信号量
